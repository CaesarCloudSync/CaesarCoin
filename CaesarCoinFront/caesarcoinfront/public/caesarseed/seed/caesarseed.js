...
window.onload= async function(){
  ...
  contributorsignup= await axios.post("https://caesarcoinbackend.onrender.com/contributorsignin",json={"contributor":contributorname,"email":"amari.lawal05@gmail.com","password":contributor_password})
  ...
  if (contributorsignup.data.access_token == "Wrong password"){
    ...
    walletbalance.innerHTML = `${contributorname} incorrect password or username.`
  }
  else{
    const config = {
      headers: { Authorization: `Bearer ${contributorsignup.data.access_token}` }
    };
    getlastblockchainresp = await axios.post("https://caesarcoinbackend.onrender.com/get_wallet_balance",json={"blockchain_name":"Caesar Block Chain","blockchain_password":blockchain_password},config)  
...
async function caesartorrent(file, i) {
  ...
  const signingresp = await axios.post('https://caesarcoinbackend.onrender.com/contributorsignin',json={"contributor":contributorname,"email":"amari.lawal05@gmail.com","password":"kya63amari"})
...
client.seed(files, async function(torrent){
  // TODO When Seeding large files it takes quite a while so use the time it takes to count the nonce as proof of work, then the general seeding will count to the extra nonce values for the block
    try{
      ...
      const storeresp = await axios.post('https://caesarcoinbackend.onrender.com/storemagneturi',json={"companyname":companyname,"quotaname":quotaname,"torrentfilename":files[0]["name"],"contributorname":contributorname,"torrentmagneturi":torrent.magnetURI,"filesize":filesize},config)
